...
Filters can still be applied in the constructor of the controller except you don't have to run the parent constructor anymore. The only difference with filters is it's now `beforeFilter()` and `afterFilter()`.
